...
use Asse\Plugin\Akamai\EdgeControl;
...
class Akamai extends AbstractPlugin
{
    ...
    {
        if (apply_filters('asse_akamai_purge_on_post', true) === false) {
            return;
        }
    ...
    public function send_headers()
    {
        ...
        if (apply_filters('asse_akamai_send_edge_control_headers_pre_check', true) === false) {
            $this->send_edge_control_header();
            return;
        }
    ...
    public function send_edge_control_header()
    {
        if (headers_sent()) {
            return;
        }

        $edge_control = new EdgeControl();
        $edge_control->max_age = !empty(get_option('asse_akamai_edge_max_age')) ? get_option('asse_akamai_edge_max_age') : $edge_control->max_age;
        $edge_control->downstream_ttl = !empty(get_option('asse_akamai_edge_downstream_ttl')) ? get_option('asse_akamai_edge_downstream_ttl') : $edge_control->downstream_ttl;
        $edge_control->cache_option = !empty(get_option('asse_akamai_edge_options')) ? get_option('asse_akamai_edge_options') : $edge_control->cache_option;

        $edge_control = apply_filters('asse_akamai_filter_edge_control', $edge_control);
    ...
    public function purge()
    {
        ...
        foreach (array_filter($hosts) as $host) {
            ...
            $responses[] = $this->do_purge($auth, $body);
        }
     ...
     */
    protected function do_purge(Authentication $auth, array $body)
    {
        return wp_remote_post('https://' . $auth->getHost() . $auth->getPath(), [
